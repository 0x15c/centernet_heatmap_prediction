...
> TL;DR: We successfully combine the CenterNet heatmap prediction and VoxelMorph 2D image registration to form a novel pipeline that is used for marker extraction in the vision-based tactile sensors (VBTS). We can extract the displacement field of markers through this proposed pipeline, making tactile force estimation much easier and more robust.

## Introduction

### Video Demo

[![Watch the video](imgs/video_demo.png)](https://youtu.be/F6eo3ldLuks)
...
### Overview
You should train CenterNet and VoxelMorph separately. CenterNet should be trained with data with YOLO-style annotations (the training script will convert the marker bounding box into a Gaussian pulse depending on its area). VoxelMorph, however, is self-supervised; just feeding it a lot of images is okay.
...
The manually crop tool is like:
![](imgs/crop.png)

> All the dataset tools are available at the `utils/` folder of this repo.

After you train the CenterNet model, you could immediately use its prediction results as a training dataset for the VoxelMorph model. This model takes a fixed and a moving grayscale image as input; you can select the pair randomly or with a permanent choice of the fixed image and a random moving image. In its training script, we add random rotations to make the model learning direction-invariant representations. 

## Next step

We would like to make this training end-to-end if time permits.
